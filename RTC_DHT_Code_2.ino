/*;;;;;;;FOR 2-NOS. LED P10 BOARD WITH RTC AND DHT11;;;;;;;;;*/
/* UTPAL CLOCK  */
#include <Wire.h>
#include <RTClib.h>
#include <SPI.h>
...
int tempr,hum;             
int i;             
char welcome_screen[] = "Welcome to RTC-DHT11 Project. "; // FOR 2 DMD'S.
RTC_DS1307 rtc;
byte SW_0 = 2;  //Tact switch
byte SW1 = 3;   //Tact switch
...
DateTime now;
char datToString[2];
DMD dmd(2,1); // DMD controls the entire display
void ScanDMD()
{
...
void drawText( String dispString )
{
  ...
  dmd.drawMarquee( newString , sLength , 64, 8);    // position of scrolling.
  long timer = millis();
   boolean ret = false;
  dmd.clearScreen(true);
   NowTimes();
  while ( !ret ) {
    if ( ( timer + 35 ) < millis() )        // 35 -change for scroll speed
    {
        NowTimes();
       ShowTime();
...
void scrollDate()
{
  String  text2, textSmntara,textSmntara1 ;
  NowTimes();
  String twodigit;
    textSmntara = message; 
    text2 += textSmntara;
    textSmntara1 = "Today is ";
    text2 += textSmntara1;
  ...
  text2 +=datToString;
   text2 += ". Now Temp-";
   ReadDht();
   TwoDigit(tempr,twodigit);
  text2 +=twodigit;
  text2 += "'C & Hum-";
  TwoDigit(hum,twodigit);
  text2 +=twodigit;
   text2 += "% .";
  drawText (text2);
}
...
{   NowTimes();
 ...
 //if(det!=0)
   //dmd.drawString(31,0,":",1,GRAPHICS_NORMAL);  //blink dot
  // else
   //dmd.drawString(31,0,":",1,GRAPHICS_NOR);
   //if(det==0)
   {
      Show_TwoDigit (4,0,now.hour());                                           // hsow hour position 16
      dmd.drawString(20,0,":",1,GRAPHICS_NORMAL);
      Show_TwoDigit (25,0,now.minute());                                        //show minute position 35
      dmd.drawString(41,0,":",1,GRAPHICS_NORMAL);
      Show_TwoDigit (46,0,now.second());}                                      //show sec position 50
 }
 ...
 void ShowTimeBig()//show big time
{
  String bigDigit="";
    ...
    int det=now.second()%2;
    //if(det!=0){
    // dmd.drawCircle(31,5,1,GRAPHICS_NORMAL); //blink clock dot
    // dmd.drawCircle(31,10,1,GRAPHICS_NORMAL);}
   
   // else
    // {dmd.drawCircle(31,5,1,GRAPHICS_NOR);
    // dmd.drawCircle(31,10,1,GRAPHICS_NOR);}
j=now.hour();
 TwoDigit(j,bigDigit);
 bigDigit.toCharArray(datToString, 3); 
 dmd.drawString(4,0,datToString,2,GRAPHICS_NORMAL); //show big hour
  dmd.drawCircle(20,5,1,GRAPHICS_NORMAL); 
  dmd.drawCircle(20,10,1,GRAPHICS_NORMAL);
     
j=now.minute();
 TwoDigit(j,bigDigit);
 bigDigit.toCharArray(datToString, 3); 
 dmd.drawString(25,0,datToString,2,GRAPHICS_NORMAL);//show big minute
  dmd.drawCircle(41,5,1,GRAPHICS_NORMAL); 
  dmd.drawCircle(41,10,1,GRAPHICS_NORMAL);
 j=now.second();
 TwoDigit(j,bigDigit);
 bigDigit.toCharArray(datToString, 3); 
 dmd.drawString(46,0,datToString,2,GRAPHICS_NORMAL);//show big minute
...
void seting()
{
 ...
 NowTimes();
  ...
  dmd.clearScreen( true );
   ...
   while (setupMode!=0){//|| (Button==2)||(Button==3)){
   delay(500);
   ...
   case 4:{                 //set bulan 
    ...
    while (Button==0){ReadButton();} //wait for key pressed
